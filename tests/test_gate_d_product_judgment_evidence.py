...
def test_local_gate_d_product_judgment_evidence_returns_exact_allowlist() -> None:
    payload = build_local_gate_d_product_judgment_evidence()

    assert type(payload) is dict
    assert payload == EXPECTED_GATE_D_PRODUCT_JUDGMENT_EVIDENCE
    assert list(payload) == list(EXPECTED_GATE_D_PRODUCT_JUDGMENT_EVIDENCE)
    assert json.loads(json.dumps(payload)) == payload
    _assert_product_judgment_evidence_output_is_safe(payload)


def test_local_gate_d_product_judgment_evidence_accepts_no_input() -> None:
    signature = inspect.signature(build_local_gate_d_product_judgment_evidence)

    assert signature.parameters == {}


def test_local_gate_d_product_judgment_evidence_source_guards() -> None:
    source = Path("src/async_scholar/gate_d_product_judgment_evidence.py").read_text(
        encoding="utf-8"
    )
...
def test_local_gate_d_product_judgment_evidence_sanitizes_malformed_payload(
    ...
    monkeypatch.setattr(
        "async_scholar.gate_d_product_judgment_evidence._build_payload",
        fake_payload,
    )
    ...
    assert str(exc_info.value) == GATE_D_PRODUCT_JUDGMENT_EVIDENCE_ERROR
    assert exc_info.value.__cause__ is None
    _assert_error_is_sanitized(str(exc_info.value))


def test_local_gate_d_product_judgment_evidence_sanitizes_internal_failures(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    def fake_payload() -> dict[str, object]:
...
def test_local_gate_d_product_judgment_evidence_output_privacy_guards() -> None:
    payload = build_local_gate_d_product_judgment_evidence()
...
def test_local_gate_d_product_judgment_evidence_does_not_touch_file_io(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
...
def _assert_product_judgment_evidence_output_is_safe(
