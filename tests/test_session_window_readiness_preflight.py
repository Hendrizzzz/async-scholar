...
from async_scholar.session_window_readiness_preflight import (
    ...
    session_window_readiness_preflight_safe_summary,
)
...
def test_build_session_window_readiness_preflight_summary_due_ready_sorted(
    tmp_path: Path,
) -> None:
    ...
    summary = build_session_window_readiness_preflight_summary(
        _stored_courses(),
        tmp_path,
        "session-001",
        "file",
    ...
    _assert_readiness_summary_is_safe(summary)


def test_build_session_window_readiness_preflight_summary_waiting_and_disabled(
    tmp_path: Path,
) -> None:
    (tmp_path / "session-001").mkdir()

    waiting = build_session_window_readiness_preflight_summary(
        _stored_courses(),
        tmp_path,
        "session-001",
        "mic",
    ...
    disabled = build_session_window_readiness_preflight_summary(
        _stored_courses(),
        tmp_path,
        "session-001",
        "file",
    ...
    assert waiting["ready_to_start"] is False
    ...
    _assert_readiness_summary_is_safe(waiting)
    _assert_readiness_summary_is_safe(disabled)


def test_session_window_readiness_preflight_safe_summary_strips_private_fields() -> (
    None
):
    safe = session_window_readiness_preflight_safe_summary(
        {
            "status": "due",
            "session_id": "session-001",
            "source_kind": "file",
    ...
    _assert_readiness_summary_is_safe(safe)


def test_session_window_readiness_preflight_safe_summary_requires_fixed_metadata() -> (
    None
):
    ...
    assert str(exc_info.value) == STORED_SESSION_WINDOW_READINESS_PREFLIGHT_ERROR
...
def test_build_session_window_readiness_preflight_summary_sanitizes_bad_inputs(
...
def test_session_window_readiness_preflight_source_has_no_execution_surfaces() -> None:
    import async_scholar.session_window_readiness_preflight as preflight
    ...
    helper_source = inspect.getsource(
        preflight.build_session_window_readiness_preflight_summary
    )
    assert "build_stored_session_window_plan_summary" in helper_source
    assert "build_archive_export_preflight_summary_from_root" in helper_source


def _assert_readiness_summary_is_safe(payload: dict[str, object]) -> None:
