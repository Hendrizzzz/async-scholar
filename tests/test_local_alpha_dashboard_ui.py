...
def test_dashboard_renders_safe_human_facing_sections() -> None:
    ...
    runbook = _find_element_by_class(
        ui,
        "async-scholar-local-alpha-dashboard__runbook",
    )
    ...
    fixture_summary_export = _find_element_by_class(
        ui,
        "async-scholar-local-alpha-dashboard__fixture-summary-export",
    )
    assert fixture_summary_export is not None
    assert [child.text for child in fixture_summary_export.children] == (
        LOCAL_ALPHA_FIXTURE_SUMMARY_EXPORT_LABELS
    )
...
def test_dashboard_inspection_summary_is_metadata_only_and_no_server() -> None:
    ...
    assert "Fixture demo summary export" in summary
    for (
...
def test_dashboard_refresh_uses_only_injected_sources() -> None:
    ...
    assert second_render.count("Private data required: no") == 2
    runbook = _find_element_by_class(
        ui,
        "async-scholar-local-alpha-dashboard__runbook",
    )
    ...
    assert {child.kind for child in runbook.children} == {"label"}
    assert all(child.on_click is None for child in runbook.children)
    assert second_render.count("Local alpha demo runbook") == 1
    assert second_render.count("1. Run fixture/local demo evidence") == 1
    assert second_render.count("2. Inspect dashboard safety status") == 1
    assert second_render.count("3. Export static local alpha dashboard") == 1
    assert second_render.count("4. Review Gate D evidence bundle") == 1
    assert second_render.count("5. Review Gate D handoff packet") == 1
    assert (
    ...
    assert {child.kind for child in fixture_summary_export.children} == {"label"}
    assert all(child.on_click is None for child in fixture_summary_export.children)
    assert second_render.count("Fixture demo summary export") == 1
    assert (
        second_render.count(
            "Summary export: scripts\\run_local_alpha_fixture_demo.ps1 "
            "-SummaryOutput <local-summary-json>"
    ...
    assert (
        second_render.count("Summary kind: local_alpha_fixture_demo_sanitized_summary")
        == 1
    )
    assert second_render.count("Fixture artifacts generated: yes") == 1
    assert second_render.count("Static dashboard generated: yes") == 1
