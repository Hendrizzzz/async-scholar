...
def test_module_help_prints_useful_output() -> None:
    ...
    assert "session-window-execute-from-store-local" in result.stdout
    assert "session-window-start-receipt-from-store-local" in result.stdout
    assert "session-window-stop-execution-preflight-from-store-local" in result.stdout
    assert "session-window-stop-execute-from-store-local" in result.stdout
...
def test_session_window_execute_from_store_local_help_stays_lazy(
    monkeypatch,
) -> None:
    scheduled_start_module = "async_scholar.scheduled_start"
    execution_module = "async_scholar.session_window_execution"
    monkeypatch.delitem(sys.modules, scheduled_start_module, raising=False)
    monkeypatch.delitem(sys.modules, execution_module, raising=False)
    ...
    assert result.returncode == 0
    assert (
        "usage: async_scholar session-window-execute-from-store-local" in result.stdout
    )
    assert "--db-path" in result.stdout
    assert "--archive-root" in result.stdout
    assert "--source-kind" in result.stdout
    ...
    assert scheduled_start_module not in sys.modules
    assert execution_module not in sys.modules


def test_session_window_execute_from_store_local_requires_metadata() -> None:
    result = subprocess.run(
        [
            sys.executable,
            "-m",
    ...
    assert result.returncode != 0
    assert result.stdout == ""
    assert result.stderr == "stored session window execution could not be built\n"


def test_session_window_execute_prints_compact_json_and_writes_runtime(
    tmp_path: Path,
) -> None:
    db_path = tmp_path / "schedule.sqlite"
    archive_root = tmp_path / "archive"
    ...
    assert result.stdout == f"{expected_line}\n"
    assert json.loads(result.stdout) == expected
    assert len(runtime_path.read_text(encoding="utf-8").splitlines()) == 1
    _assert_session_window_execute_output_is_safe(
        result.stdout,
        result.stderr,
        runtime_path,
    )
...
def test_session_window_execute_prints_blocked_json_without_runtime_write(
    tmp_path: Path,
) -> None:
    db_path = tmp_path / "schedule.sqlite"
    archive_root = tmp_path / "archive"
    ...
    assert payload["preflight_decision"] == "block"
    assert payload["decision"] == "blocked"
    assert payload["reason"] == "confirmation_declined"
    assert not (session_dir / "runtime.jsonl").exists()
    _assert_session_window_execute_output_is_safe(result.stdout, result.stderr)


def test_session_window_execute_sanitizes_parse_failure() -> None:
    result = subprocess.run(
        [
            sys.executable,
            "-m",
...
def test_session_window_execute_sanitizes_runtime_failure(
    tmp_path: Path,
) -> None:
    db_path = tmp_path / "schedule.sqlite"
    archive_root = tmp_path / "archive-token-secret-auth-profile"
...
def test_session_window_execute_misordered_uses_execution_error() -> None:
    result = subprocess.run(
        [
            sys.executable,
            "-m",
...
def test_session_window_execute_command_delegates_to_helper(
    capsys,
    monkeypatch,
) -> None:
    received: dict[str, object] = {}
    ...
    execution_module = "async_scholar.session_window_execution"
    fake_scheduled_start_module = types.ModuleType(scheduled_start_module)
    fake_execution_module = types.ModuleType(execution_module)
    fake_clock = object()
    ...
    fake_scheduled_start_module.ScheduledStartClock = FakeClock
    fake_execution_module.build_stored_session_window_execution_from_store = fake_build
    monkeypatch.setitem(
        sys.modules,
        scheduled_start_module,
        fake_scheduled_start_module,
    )
    monkeypatch.setitem(sys.modules, execution_module, fake_execution_module)
    ...
    assert exit_code == 0
    assert captured.err == ""
    assert json.loads(captured.out)["decision"] == "executed"
    assert received == {
        "clock_kwargs": {"day_of_week": "monday", "local_time": "09:00"},
        "db_path": Path("schedule.sqlite"),
        "archive_root": Path("archive-root"),
...
def test_session_window_execute_handler_stays_thin() -> None:
    source = inspect.getsource(cli._run_session_window_execute_from_store_local_command)
...
def test_session_window_stop_execution_preflight_help_stays_lazy(
    monkeypatch,
) -> None:
    execution_module = "async_scholar.session_window_stop_execution_preflight"
    monkeypatch.delitem(sys.modules, execution_module, raising=False)
    ...
    assert result.returncode == 0
    assert (
        "usage: async_scholar session-window-stop-execution-preflight-from-store-local"
    ) in result.stdout
    assert "--db-path" in result.stdout
    assert "--archive-root" in result.stdout
    assert "--course-id" in result.stdout
...
def test_session_window_stop_execution_preflight_requires_metadata() -> None:
    result = subprocess.run(
        [
            sys.executable,
            "-m",
...
def test_session_window_stop_execution_preflight_prints_allow_json(
    tmp_path: Path,
) -> None:
    db_path = tmp_path / "schedule.sqlite"
    archive_root = tmp_path / "archive"
    ...
    runtime_line = json.dumps(
        _session_window_runtime_start_receipt(),
        sort_keys=True,
        separators=(",", ":"),
    )
    runtime_path.write_text(f"{runtime_line}\n", encoding="utf-8")
    ...
    assert result.stderr == ""
    assert result.stdout == f"{expected_line}\n"
    assert runtime_path.read_text(encoding="utf-8") == f"{runtime_line}\n"
    _assert_session_window_stop_execution_preflight_output_is_safe(
        result.stdout,
        result.stderr,
    )


def test_session_window_stop_execution_preflight_prints_blocked_json(
    tmp_path: Path,
) -> None:
    db_path = tmp_path / "schedule.sqlite"
    archive_root = tmp_path / "archive"
    ...
    assert result.returncode == 0
    assert result.stderr == ""
    assert payload["runtime_state"] == "missing"
    assert payload["ready_to_stop"] is False
    assert payload["decision"] == "block"
...
def test_session_window_stop_execution_preflight_sanitizes_parse_failure() -> None:
    result = subprocess.run(
        [
            sys.executable,
            "-m",
...
def test_session_window_stop_execution_preflight_sanitizes_runtime_failure(
    tmp_path: Path,
) -> None:
    db_path = tmp_path / "schedule.sqlite"
    archive_root = tmp_path / "archive-token-secret-auth-profile"
...
def test_session_window_stop_execution_preflight_misordered_uses_preflight_error() -> (
    None
):
    result = subprocess.run(
        [
...
def test_session_window_stop_execution_preflight_command_delegates_to_helper(
    capsys,
    monkeypatch,
) -> None:
    received: dict[str, object] = {}
    preflight_module = "async_scholar.session_window_stop_execution_preflight"
    fake_preflight_module = types.ModuleType(preflight_module)
    ...
    helper_name = "build_stored_session_window_stop_execution_preflight_from_store"
    fake_preflight_module.__dict__[helper_name] = fake_build
    monkeypatch.setitem(sys.modules, preflight_module, fake_preflight_module)
    ...
    assert exit_code == 0
    assert captured.err == ""
    assert json.loads(captured.out)["ready_to_stop"] is True
    assert received == {
        "db_path": Path("schedule.sqlite"),
        "archive_root": Path("archive-root"),
        "session_id": "session-001",
...
def test_session_window_stop_execution_preflight_handler_stays_thin() -> None:
    source = inspect.getsource(
        cli._run_session_window_stop_execution_preflight_from_store_local_command
    )
...
def test_session_window_stop_execute_from_store_local_help_stays_lazy(
    monkeypatch,
) -> None:
    execution_module = "async_scholar.session_window_stop_execution"
    monkeypatch.delitem(sys.modules, execution_module, raising=False)
    ...
    assert result.returncode == 0
    assert (
        "usage: async_scholar session-window-stop-execute-from-store-local"
    ) in result.stdout
    assert "--db-path" in result.stdout
    assert "--archive-root" in result.stdout
    assert "--course-id" in result.stdout
...
def test_session_window_stop_execute_from_store_local_requires_metadata() -> None:
    result = subprocess.run(
        [
            sys.executable,
            "-m",
    ...
    assert result.returncode != 0
    assert result.stdout == ""
    assert result.stderr == "stored session window stop execution could not be built\n"


def test_session_window_stop_execute_prints_executed_json_and_appends_runtime(
    tmp_path: Path,
) -> None:
    db_path = tmp_path / "schedule.sqlite"
    archive_root = tmp_path / "archive"
    ...
    assert json.loads(runtime_lines[1])["receipt_kind"] == (
        "stored_session_window_stop_receipt"
    )
    _assert_session_window_stop_execute_output_is_safe(
        result.stdout,
        result.stderr,
        runtime_path,
    )
...
def test_session_window_stop_execute_declined_does_not_append_runtime(
    tmp_path: Path,
) -> None:
    db_path = tmp_path / "schedule.sqlite"
    archive_root = tmp_path / "archive"
    ...
    assert result.returncode == 0
    assert result.stderr == ""
    assert payload["preflight_decision"] == "allow"
    assert payload["decision"] == "blocked"
    assert payload["reason"] == "confirmation_declined"
    assert payload["runtime_record_written"] is False
    assert runtime_path.read_text(encoding="utf-8") == f"{runtime_line}\n"
...
def test_session_window_stop_execute_prints_blocked_preflight_json(
    tmp_path: Path,
) -> None:
    db_path = tmp_path / "schedule.sqlite"
    archive_root = tmp_path / "archive"
...
def test_session_window_stop_execute_sanitizes_parse_failure() -> None:
    result = subprocess.run(
        [
            sys.executable,
            "-m",
...
def test_session_window_stop_execute_misordered_uses_execution_error() -> None:
    result = subprocess.run(
        [
            sys.executable,
            "-m",
...
def test_session_window_stop_execute_command_delegates_to_helper(
    capsys,
    monkeypatch,
) -> None:
    received: dict[str, object] = {}
    ...
    helper_name = "build_stored_session_window_stop_execution_from_store"
    fake_execution_module.__dict__[helper_name] = fake_build
    monkeypatch.setitem(sys.modules, execution_module, fake_execution_module)
...
def test_session_window_stop_execute_command_sanitizes_helper_failure(
    capsys,
    monkeypatch,
) -> None:
    execution_module = "async_scholar.session_window_stop_execution"
    ...
    assert exit_code == 1
    assert captured.out == ""
    assert captured.err == "stored session window stop execution could not be built\n"
    for forbidden_fragment in (
        "C:\\Users",
        "student",
        "token",
...
def test_session_window_stop_execute_handler_stays_thin() -> None:
    source = inspect.getsource(
        cli._run_session_window_stop_execute_from_store_local_command
    )
...
def test_gate_d_security_review_evidence_local_help_stays_lazy(monkeypatch) -> None:
    module_name = "async_scholar.gate_d_security_review_evidence"
    monkeypatch.delitem(sys.modules, module_name, raising=False)
    ...
    assert result.returncode == 0
    assert "usage: async_scholar gate-d-security-review-evidence-local" in (
        result.stdout
    )
    assert module_name not in sys.modules


def test_gate_d_security_review_evidence_local_prints_compact_json() -> None:
    result = subprocess.run(
        [
            sys.executable,
            "-m",
    ...
    assert result.stderr == ""
    assert result.stdout == f"{expected_line}\n"
    _assert_gate_d_security_review_evidence_output_is_safe(
        result.stdout,
        result.stderr,
    )


def test_gate_d_security_review_evidence_local_rejects_extra_arguments_safely() -> None:
    result = subprocess.run(
        [
            sys.executable,
            "-m",
    ...
    assert result.returncode == 2
    assert result.stdout == ""
    assert result.stderr == "gate d security review evidence could not be built\n"
    for forbidden_fragment in (
        "C:\\Users",
        "student",
        "token",
...
def test_gate_d_security_review_evidence_local_misordered_uses_fixed_error() -> None:
    result = subprocess.run(
        [
            sys.executable,
            "-m",
...
def test_gate_d_security_review_evidence_command_delegates_to_helper(
    capsys,
    monkeypatch,
) -> None:
    received: dict[str, bool] = {}
    ...
    def fake_build_local_gate_d_security_review_evidence() -> dict[str, object]:
        received["called"] = True
        return EXPECTED_GATE_D_SECURITY_REVIEW_EVIDENCE

    fake_module.build_local_gate_d_security_review_evidence = (
        fake_build_local_gate_d_security_review_evidence
    )
    monkeypatch.setitem(sys.modules, module_name, fake_module)

    exit_code = cli.main(["gate-d-security-review-evidence-local"])
    captured = capsys.readouterr()

    assert exit_code == 0
    assert received == {"called": True}
    assert captured.err == ""
    assert json.loads(captured.out) == EXPECTED_GATE_D_SECURITY_REVIEW_EVIDENCE


def test_gate_d_security_review_evidence_local_sanitizes_helper_failure(
    capsys,
    monkeypatch,
) -> None:
    module_name = "async_scholar.gate_d_security_review_evidence"
    ...
    assert exit_code == 1
    assert captured.out == ""
    assert captured.err == "gate d security review evidence could not be built\n"
    for forbidden_fragment in (
        "C:\\Users",
        "student",
        ".env",
...
def test_gate_d_security_review_evidence_local_sanitizes_import_failure(
    capsys,
    monkeypatch,
) -> None:
    real_import = builtins.__import__
    ...
    def fake_import(name, globals=None, locals=None, fromlist=(), level=0):
        if name == "async_scholar.gate_d_security_review_evidence":
            raise ImportError(
                "C:\\Users\\student\\.env BOT_TOKEN=secret import traceback"
            )
        return real_import(name, globals, locals, fromlist, level)
...
def test_gate_d_security_review_evidence_local_sanitizes_malformed_helper_output(
    capsys,
    monkeypatch,
) -> None:
    module_name = "async_scholar.gate_d_security_review_evidence"
...
def test_gate_d_security_review_evidence_local_sanitizes_json_malformed_output(
    capsys,
    monkeypatch,
) -> None:
    module_name = "async_scholar.gate_d_security_review_evidence"
...
def test_gate_d_security_review_evidence_local_handler_stays_thin() -> None:
    source = inspect.getsource(cli._run_gate_d_security_review_evidence_local_command)
...
def test_gate_d_signal_quality_evidence_local_help_stays_lazy(monkeypatch) -> None:
    module_name = "async_scholar.gate_d_signal_quality_evidence"
    monkeypatch.delitem(sys.modules, module_name, raising=False)
    ...
    assert result.returncode == 0
    assert "usage: async_scholar gate-d-signal-quality-evidence-local" in (
        result.stdout
    )
    assert module_name not in sys.modules


def test_gate_d_signal_quality_evidence_local_prints_compact_json() -> None:
    result = subprocess.run(
        [
            sys.executable,
            "-m",
    ...
    assert result.stderr == ""
    assert result.stdout == f"{expected_line}\n"
    _assert_gate_d_signal_quality_evidence_output_is_safe(
        result.stdout,
        result.stderr,
    )


def test_gate_d_signal_quality_evidence_local_rejects_extra_args_safely() -> None:
    result = subprocess.run(
        [
            sys.executable,
            "-m",
    ...
    assert result.returncode == 2
    assert result.stdout == ""
    assert result.stderr == "gate d signal quality evidence could not be built\n"
    for forbidden_fragment in (
        "C:\\Users",
        "student",
        "token",
...
def test_gate_d_signal_quality_evidence_local_misordered_uses_error() -> None:
    result = subprocess.run(
        [
            sys.executable,
            "-m",
...
def test_gate_d_signal_quality_evidence_command_delegates_to_helper(
    capsys,
    monkeypatch,
) -> None:
    received: dict[str, bool] = {}
    ...
    def fake_build_local_gate_d_signal_quality_evidence() -> dict[str, object]:
        received["called"] = True
        return EXPECTED_GATE_D_SIGNAL_QUALITY_EVIDENCE

    fake_module.build_local_gate_d_signal_quality_evidence = (
        fake_build_local_gate_d_signal_quality_evidence
    )
    monkeypatch.setitem(sys.modules, module_name, fake_module)

    exit_code = cli.main(["gate-d-signal-quality-evidence-local"])
    captured = capsys.readouterr()

    assert exit_code == 0
    assert received == {"called": True}
    assert captured.err == ""
    assert json.loads(captured.out) == EXPECTED_GATE_D_SIGNAL_QUALITY_EVIDENCE


def test_gate_d_signal_quality_evidence_local_sanitizes_helper_failure(
    capsys,
    monkeypatch,
) -> None:
    module_name = "async_scholar.gate_d_signal_quality_evidence"
    ...
    assert exit_code == 1
    assert captured.out == ""
    assert captured.err == "gate d signal quality evidence could not be built\n"
    for forbidden_fragment in (
        "C:\\Users",
        "student",
        ".env",
...
def test_gate_d_signal_quality_evidence_local_sanitizes_import_failure(
    capsys,
    monkeypatch,
) -> None:
    real_import = builtins.__import__
    ...
    def fake_import(name, globals=None, locals=None, fromlist=(), level=0):
        if name == "async_scholar.gate_d_signal_quality_evidence":
            raise ImportError(
                "C:\\Users\\student\\.env BOT_TOKEN=secret import traceback"
            )
        return real_import(name, globals, locals, fromlist, level)
...
def test_gate_d_signal_quality_evidence_local_sanitizes_malformed_output(
    capsys,
    monkeypatch,
) -> None:
    module_name = "async_scholar.gate_d_signal_quality_evidence"
...
def test_gate_d_signal_quality_evidence_local_sanitizes_json_malformed_output(
    capsys,
    monkeypatch,
) -> None:
    module_name = "async_scholar.gate_d_signal_quality_evidence"
...
def test_gate_d_signal_quality_evidence_local_handler_stays_thin() -> None:
    source = inspect.getsource(cli._run_gate_d_signal_quality_evidence_local_command)
...
EXPECTED_GATE_D_PRODUCT_JUDGMENT_EVIDENCE = {
...
def test_gate_d_product_judgment_evidence_local_help_stays_lazy(
    monkeypatch,
) -> None:
    module_name = "async_scholar.gate_d_product_judgment_evidence"
    monkeypatch.delitem(sys.modules, module_name, raising=False)
    ...
    assert result.returncode == 0
    assert (
        "usage: async_scholar gate-d-product-judgment-evidence-local" in result.stdout
    )
    assert module_name not in sys.modules


def test_gate_d_product_judgment_evidence_local_prints_compact_json() -> None:
    result = subprocess.run(
        [
            sys.executable,
            "-m",
    ...
    assert result.stderr == ""
    assert result.stdout == f"{expected_line}\n"
    _assert_gate_d_product_judgment_evidence_output_is_safe(
        result.stdout,
        result.stderr,
    )


def test_gate_d_product_judgment_evidence_local_rejects_extra_args_safely() -> None:
    result = subprocess.run(
        [
            sys.executable,
            "-m",
    ...
    assert result.returncode == 2
    assert result.stdout == ""
    assert result.stderr == "gate d product judgment evidence could not be built\n"
    for forbidden_fragment in (
        "C:\\Users",
        "student",
        "token",
...
def test_gate_d_product_judgment_evidence_local_misordered_uses_error() -> None:
    result = subprocess.run(
        [
            sys.executable,
            "-m",
...
def test_gate_d_product_judgment_evidence_command_delegates_to_helper(
    capsys,
    monkeypatch,
) -> None:
    received: dict[str, bool] = {}
    ...
    def fake_build_local_gate_d_product_judgment_evidence() -> dict[str, object]:
        received["called"] = True
        return EXPECTED_GATE_D_PRODUCT_JUDGMENT_EVIDENCE

    fake_module.build_local_gate_d_product_judgment_evidence = (
        fake_build_local_gate_d_product_judgment_evidence
    )
    monkeypatch.setitem(sys.modules, module_name, fake_module)

    exit_code = cli.main(["gate-d-product-judgment-evidence-local"])
    captured = capsys.readouterr()

    assert exit_code == 0
    assert received == {"called": True}
    assert captured.err == ""
    assert json.loads(captured.out) == EXPECTED_GATE_D_PRODUCT_JUDGMENT_EVIDENCE


def test_gate_d_product_judgment_evidence_local_sanitizes_helper_failure(
    capsys,
    monkeypatch,
) -> None:
    module_name = "async_scholar.gate_d_product_judgment_evidence"
    ...
    assert exit_code == 1
    assert captured.out == ""
    assert captured.err == "gate d product judgment evidence could not be built\n"
    for forbidden_fragment in (
        "C:\\Users",
        "student",
        ".env",
...
def test_gate_d_product_judgment_evidence_local_sanitizes_import_failure(
    capsys,
    monkeypatch,
) -> None:
    real_import = builtins.__import__
    ...
    def fake_import(name, globals=None, locals=None, fromlist=(), level=0):
        if name == "async_scholar.gate_d_product_judgment_evidence":
            raise ImportError(
                "C:\\Users\\student\\.env BOT_TOKEN=secret import traceback"
            )
        return real_import(name, globals, locals, fromlist, level)
...
def test_gate_d_product_judgment_evidence_local_sanitizes_malformed_output(
    capsys,
    monkeypatch,
) -> None:
    module_name = "async_scholar.gate_d_product_judgment_evidence"
...
def test_gate_d_product_judgment_evidence_local_sanitizes_json_malformed_output(
    capsys,
    monkeypatch,
) -> None:
    module_name = "async_scholar.gate_d_product_judgment_evidence"
...
def test_gate_d_product_judgment_evidence_local_handler_stays_thin() -> None:
    source = inspect.getsource(cli._run_gate_d_product_judgment_evidence_local_command)
...
def test_gate_d_product_judgment_packet_local_help_stays_lazy(
    monkeypatch,
) -> None:
    module_name = "async_scholar.gate_d_product_judgment_packet"
    monkeypatch.delitem(sys.modules, module_name, raising=False)
    ...
    assert result.returncode == 0
    assert "usage: async_scholar gate-d-product-judgment-packet-local" in (
        result.stdout
    )
    assert module_name not in sys.modules


def test_gate_d_product_judgment_packet_local_prints_compact_json() -> None:
    result = subprocess.run(
        [
            sys.executable,
            "-m",
    ...
    assert result.stderr == ""
    assert result.stdout == f"{expected_line}\n"
    _assert_gate_d_product_judgment_packet_output_is_safe(
        result.stdout,
        result.stderr,
    )


def test_gate_d_product_judgment_packet_local_rejects_extra_args_safely() -> None:
    result = subprocess.run(
        [
            sys.executable,
            "-m",
    ...
    assert result.returncode == 2
    assert result.stdout == ""
    assert result.stderr == "gate d product judgment packet could not be built\n"
    for forbidden_fragment in (
        "C:\\Users",
        "student",
        "token",
...
def test_gate_d_product_judgment_packet_local_misordered_uses_error() -> None:
    result = subprocess.run(
        [
            sys.executable,
            "-m",
...
def test_gate_d_product_judgment_packet_command_delegates_to_helper(
    capsys,
    monkeypatch,
) -> None:
    received: dict[str, bool] = {}
    ...
    def fake_build_local_gate_d_product_judgment_packet() -> dict[str, object]:
        received["called"] = True
        return EXPECTED_GATE_D_PRODUCT_JUDGMENT_PACKET

    fake_module.build_local_gate_d_product_judgment_packet = (
        fake_build_local_gate_d_product_judgment_packet
    )
    monkeypatch.setitem(sys.modules, module_name, fake_module)

    exit_code = cli.main(["gate-d-product-judgment-packet-local"])
    captured = capsys.readouterr()

    assert exit_code == 0
    assert received == {"called": True}
    assert captured.err == ""
    assert json.loads(captured.out) == EXPECTED_GATE_D_PRODUCT_JUDGMENT_PACKET


def test_gate_d_product_judgment_packet_local_sanitizes_helper_failure(
    capsys,
    monkeypatch,
) -> None:
    module_name = "async_scholar.gate_d_product_judgment_packet"
    ...
    assert exit_code == 1
    assert captured.out == ""
    assert captured.err == "gate d product judgment packet could not be built\n"
    for forbidden_fragment in (
        "C:\\Users",
        "student",
        ".env",
...
def test_gate_d_product_judgment_packet_local_sanitizes_import_failure(
    capsys,
    monkeypatch,
) -> None:
    real_import = builtins.__import__
    ...
    def fake_import(name, globals=None, locals=None, fromlist=(), level=0):
        if name == "async_scholar.gate_d_product_judgment_packet":
            raise ImportError(
                "C:\\Users\\student\\.env BOT_TOKEN=secret import traceback"
            )
        return real_import(name, globals, locals, fromlist, level)
...
def test_gate_d_product_judgment_packet_local_sanitizes_malformed_output(
    capsys,
    monkeypatch,
) -> None:
    module_name = "async_scholar.gate_d_product_judgment_packet"
...
def test_gate_d_product_judgment_packet_local_sanitizes_json_malformed_output(
    capsys,
    monkeypatch,
) -> None:
    module_name = "async_scholar.gate_d_product_judgment_packet"
...
def test_gate_d_product_judgment_packet_local_handler_stays_thin() -> None:
    source = inspect.getsource(cli._run_gate_d_product_judgment_packet_local_command)
...
def test_gate_d_scheduler_lifecycle_evidence_local_help_stays_lazy(
    monkeypatch,
) -> None:
    module_name = "async_scholar.gate_d_scheduler_lifecycle_evidence"
    monkeypatch.delitem(sys.modules, module_name, raising=False)
    ...
    assert result.returncode == 0
    assert "usage: async_scholar gate-d-scheduler-lifecycle-evidence-local" in (
        result.stdout
    )
    assert module_name not in sys.modules


def test_gate_d_scheduler_lifecycle_evidence_local_prints_compact_json() -> None:
    result = subprocess.run(
        [
            sys.executable,
            "-m",
    ...
    assert result.stderr == ""
    assert result.stdout == f"{expected_line}\n"
    _assert_gate_d_scheduler_lifecycle_evidence_output_is_safe(
        result.stdout,
        result.stderr,
    )


def test_gate_d_scheduler_lifecycle_evidence_local_rejects_extra_args_safely() -> None:
    result = subprocess.run(
        [
            sys.executable,
            "-m",
    ...
    assert result.returncode == 2
    assert result.stdout == ""
    assert result.stderr == ("gate d scheduler lifecycle evidence could not be built\n")
    for forbidden_fragment in (
        "C:\\Users",
        "student",
        "token",
...
def test_gate_d_scheduler_lifecycle_evidence_local_misordered_uses_error() -> None:
    result = subprocess.run(
        [
            sys.executable,
            "-m",
...
def test_gate_d_scheduler_lifecycle_evidence_command_delegates_to_helper(
    capsys,
    monkeypatch,
) -> None:
    received: dict[str, bool] = {}
    ...
    def fake_build_local_gate_d_scheduler_lifecycle_evidence() -> dict[str, object]:
        received["called"] = True
        return EXPECTED_GATE_D_SCHEDULER_LIFECYCLE_EVIDENCE

    fake_module.build_local_gate_d_scheduler_lifecycle_evidence = (
        fake_build_local_gate_d_scheduler_lifecycle_evidence
    )
    monkeypatch.setitem(sys.modules, module_name, fake_module)

    exit_code = cli.main(["gate-d-scheduler-lifecycle-evidence-local"])
    captured = capsys.readouterr()

    assert exit_code == 0
    assert received == {"called": True}
    assert captured.err == ""
    assert json.loads(captured.out) == EXPECTED_GATE_D_SCHEDULER_LIFECYCLE_EVIDENCE


def test_gate_d_scheduler_lifecycle_evidence_local_sanitizes_helper_failure(
    capsys,
    monkeypatch,
) -> None:
    module_name = "async_scholar.gate_d_scheduler_lifecycle_evidence"
    ...
    assert exit_code == 1
    assert captured.out == ""
    assert captured.err == ("gate d scheduler lifecycle evidence could not be built\n")
    for forbidden_fragment in (
        "C:\\Users",
        "student",
        ".env",
...
def test_gate_d_scheduler_lifecycle_evidence_local_sanitizes_import_failure(
    capsys,
    monkeypatch,
) -> None:
    real_import = builtins.__import__
    ...
    def fake_import(name, globals=None, locals=None, fromlist=(), level=0):
        if name == "async_scholar.gate_d_scheduler_lifecycle_evidence":
            raise ImportError(
                "C:\\Users\\student\\.env BOT_TOKEN=secret import traceback"
            )
        return real_import(name, globals, locals, fromlist, level)
...
def test_gate_d_scheduler_lifecycle_evidence_local_sanitizes_malformed_output(
    capsys,
    monkeypatch,
) -> None:
    module_name = "async_scholar.gate_d_scheduler_lifecycle_evidence"
...
def test_gate_d_scheduler_lifecycle_evidence_local_sanitizes_json_malformed_output(
    capsys,
    monkeypatch,
) -> None:
    module_name = "async_scholar.gate_d_scheduler_lifecycle_evidence"
...
def test_gate_d_scheduler_lifecycle_evidence_local_handler_stays_thin() -> None:
    source = inspect.getsource(
        cli._run_gate_d_scheduler_lifecycle_evidence_local_command
    )
...
def test_gate_d_local_evidence_bundle_help_stays_lazy(monkeypatch) -> None:
    module_name = "async_scholar.gate_d_local_evidence_bundle"
    monkeypatch.delitem(sys.modules, module_name, raising=False)
    ...
    assert result.returncode == 0
    assert "usage: async_scholar gate-d-local-evidence-bundle" in result.stdout
    assert module_name not in sys.modules


def test_gate_d_local_evidence_bundle_prints_compact_json() -> None:
    result = subprocess.run(
        [sys.executable, "-m", "async_scholar", "gate-d-local-evidence-bundle"],
        check=False,
        capture_output=True,
        text=True,
    )
    ...
    assert result.stderr == ""
    assert result.stdout == f"{expected_line}\n"
    _assert_gate_d_local_evidence_bundle_output_is_safe(
        result.stdout,
        result.stderr,
    )


def test_gate_d_local_evidence_bundle_rejects_extra_arguments_safely() -> None:
    result = subprocess.run(
        [
            sys.executable,
            "-m",
    ...
    assert result.returncode == 2
    assert result.stdout == ""
    assert result.stderr == "gate d local evidence bundle could not be built\n"
    for forbidden_fragment in (
        "C:\\Users",
        "student",
        "token",
...
def test_gate_d_local_evidence_bundle_misordered_uses_bundle_error() -> None:
    result = subprocess.run(
        [
            sys.executable,
            "-m",
...
def test_gate_d_local_evidence_bundle_command_delegates_to_helper(
    capsys,
    monkeypatch,
) -> None:
    received: dict[str, bool] = {}
    ...
    def fake_build_local_gate_d_smoke_evidence_bundle() -> dict[str, object]:
        received["called"] = True
        return EXPECTED_GATE_D_LOCAL_EVIDENCE_BUNDLE

    fake_module.build_local_gate_d_smoke_evidence_bundle = (
        fake_build_local_gate_d_smoke_evidence_bundle
    )
    monkeypatch.setitem(sys.modules, module_name, fake_module)

    exit_code = cli.main(["gate-d-local-evidence-bundle"])
    captured = capsys.readouterr()

    assert exit_code == 0
    assert received == {"called": True}
    assert captured.err == ""
    assert json.loads(captured.out) == EXPECTED_GATE_D_LOCAL_EVIDENCE_BUNDLE


def test_gate_d_local_evidence_bundle_sanitizes_helper_failure(
    capsys,
    monkeypatch,
) -> None:
    module_name = "async_scholar.gate_d_local_evidence_bundle"
    ...
    assert exit_code == 1
    assert captured.out == ""
    assert captured.err == "gate d local evidence bundle could not be built\n"
    for forbidden_fragment in (
        "C:\\Users",
        "student",
        ".env",
...
def test_gate_d_local_evidence_bundle_sanitizes_import_failure(
    capsys,
    monkeypatch,
) -> None:
    real_import = builtins.__import__
    ...
    def fake_import(name, globals=None, locals=None, fromlist=(), level=0):
        if name == "async_scholar.gate_d_local_evidence_bundle":
            raise ImportError(
                "C:\\Users\\student\\.env BOT_TOKEN=secret import traceback"
            )
        return real_import(name, globals, locals, fromlist, level)
...
def test_gate_d_local_evidence_bundle_sanitizes_malformed_helper_output(
    capsys,
    monkeypatch,
) -> None:
    module_name = "async_scholar.gate_d_local_evidence_bundle"
...
def test_gate_d_local_evidence_bundle_sanitizes_json_serializable_malformed_output(
    capsys,
    monkeypatch,
) -> None:
    module_name = "async_scholar.gate_d_local_evidence_bundle"
...
def test_gate_d_local_evidence_bundle_handler_stays_thin() -> None:
    source = inspect.getsource(cli._run_gate_d_local_evidence_bundle_command)
...
def test_session_window_recovery_review_batch_local_help_stays_lazy(
    monkeypatch,
) -> None:
    batch_module = "async_scholar.session_window_recovery_batch_review"
    monkeypatch.delitem(sys.modules, batch_module, raising=False)
    ...
    assert result.returncode == 0
    assert "usage: async_scholar session-window-recovery-review-batch-local" in (
        result.stdout
    )
    assert "--archive-root" in result.stdout
    assert "read-only" in result.stdout
    assert "recovery review batch" in result.stdout
    assert batch_module not in sys.modules


def test_session_window_recovery_review_batch_local_requires_metadata() -> None:
    result = subprocess.run(
        [
            sys.executable,
            "-m",
...
def test_session_window_recovery_review_batch_command_delegates_to_builder(
    capsys,
    monkeypatch,
) -> None:
    received: dict[str, object] = {}
    batch_module = "async_scholar.session_window_recovery_batch_review"
    fake_batch_module = types.ModuleType(batch_module)
    ...
    fake_batch_module.build_stored_session_window_recovery_review_batch = fake_build
    monkeypatch.setitem(sys.modules, batch_module, fake_batch_module)
    ...
    assert captured.out == (
        '{"batch_kind":"stored_session_window_recovery_review_batch",'
        '"manual_review_required_count":1,"not_required_count":1,'
        '"required_count":1,"review_count":2,"reviews":[{'
        '"archive_existing_count":0,"archive_missing_count":6,'
        '"archive_recovery_status":"empty","manual_review_required":false,'
        '"recovery_decision":"no_action",'
        '"review_kind":"stored_session_window_recovery_review",'
        ...
        '"session_id":"session-001"},{'
        '"archive_existing_count":0,"archive_missing_count":6,'
        '"archive_recovery_status":"empty","manual_review_required":true,'
        '"recovery_decision":"inspect_active_session",'
        '"review_kind":"stored_session_window_recovery_review",'
        '"review_reason":"active_session_runtime","review_status":"required",'
        '"runtime_lifecycle_status":"started",'
        '"safe_next_review_action":"inspect_runtime_metadata",'
        '"session_id":"session-002"}]}\n'
    )
...
def test_session_window_recovery_review_batch_sanitizes_build_failure(
    capsys,
    monkeypatch,
) -> None:
    batch_module = "async_scholar.session_window_recovery_batch_review"
...
def test_session_window_recovery_review_batch_misordered_uses_batch_error() -> None:
    result = subprocess.run(
        [
            sys.executable,
            "-m",
...
def test_session_window_recovery_report_file_inventory_local_help_stays_lazy(
    monkeypatch,
) -> None:
    inventory_module = "async_scholar.session_window_recovery_report_file_inventory"
    monkeypatch.delitem(sys.modules, inventory_module, raising=False)
    ...
    assert result.returncode == 0
    assert (
        "usage: async_scholar session-window-recovery-report-file-inventory-local"
    ) in result.stdout
    assert "--archive-root" in result.stdout
    assert "--output-root" in result.stdout
    assert "inventory" in result.stdout
    assert "recovery report" in result.stdout
    assert inventory_module not in sys.modules


def test_session_window_recovery_report_file_inventory_local_requires_metadata() -> (
    None
):
    result = subprocess.run(
        [
...
def test_session_window_recovery_report_file_inventory_command_delegates_to_builder(
    capsys,
    monkeypatch,
) -> None:
    received: dict[str, object] = {}
    inventory_module = "async_scholar.session_window_recovery_report_file_inventory"
    fake_inventory_module = types.ModuleType(inventory_module)

    def fake_build(archive_root: Path, output_root: Path) -> dict[str, object]:
        received["archive_root"] = archive_root
        received["output_root"] = output_root
        return {
    ...
    fake_inventory_module.build_stored_session_window_recovery_report_file_inventory = (
        fake_build
    )
    monkeypatch.setitem(sys.modules, inventory_module, fake_inventory_module)
...
def test_session_window_recovery_report_file_inventory_sanitizes_build_failure(
    capsys,
    monkeypatch,
) -> None:
    inventory_module = "async_scholar.session_window_recovery_report_file_inventory"
...
def test_session_window_report_file_inventory_misordered_uses_inventory_error() -> None:
    result = subprocess.run(
        [
            sys.executable,
            "-m",
...
def test_session_window_recovery_report_file_inventory_handler_stays_thin() -> None:
    source = inspect.getsource(
        cli._run_session_window_recovery_report_file_inventory_local_command
    )
...
def test_session_window_recovery_report_file_action_preview_local_help_stays_lazy(
    monkeypatch,
) -> None:
    preview_module = "async_scholar.session_window_recovery_report_file_action_preview"
    ...
    assert result.returncode == 0
    assert (
        "usage: async_scholar session-window-recovery-report-file-action-preview-local"
    ) in result.stdout
    assert "--archive-root" in result.stdout
    assert "--output-root" in result.stdout
    assert "preview" in result.stdout
    assert "recovery report" in result.stdout
    assert preview_module not in sys.modules


def test_recovery_report_file_action_preview_local_requires_metadata() -> None:
    result = subprocess.run(
        [
            sys.executable,
            "-m",
...
def test_recovery_report_file_action_preview_command_delegates_to_builder(
    capsys,
    monkeypatch,
) -> None:
    received: dict[str, object] = {}
    preview_module = "async_scholar.session_window_recovery_report_file_action_preview"
    fake_preview_module = types.ModuleType(preview_module)
    ...
    def fake_build(
        ...
        return {
            "preview_kind": (
                "stored_session_window_recovery_report_file_action_preview"
            ),
            "session_count": 1,
            "relative_path": "stored-session-window-recovery-report.md",
            "exists": True,
    ...
    fake_preview_module.__dict__[
        "build_stored_session_window_recovery_report_file_action_preview"
    ] = fake_build
    monkeypatch.setitem(sys.modules, preview_module, fake_preview_module)
...
def test_session_window_recovery_report_file_action_preview_sanitizes_build_failure(
    capsys,
    monkeypatch,
) -> None:
    preview_module = "async_scholar.session_window_recovery_report_file_action_preview"
...
def test_session_window_report_file_action_preview_misordered_uses_preview_error() -> (
    None
):
    result = subprocess.run(
        [
...
def test_session_window_recovery_report_file_action_preview_handler_stays_thin() -> (
    None
):
    source = inspect.getsource(
        cli._run_session_window_recovery_report_file_action_preview_local_command
    )
...
def test_session_window_recovery_report_file_action_local_help_stays_lazy(
    monkeypatch,
) -> None:
    action_module = "async_scholar.session_window_recovery_report_file_action"
    monkeypatch.delitem(sys.modules, action_module, raising=False)
    ...
    assert result.returncode == 0
    assert (
        "usage: async_scholar session-window-recovery-report-file-action-local"
    ) in result.stdout
    assert "--archive-root" in result.stdout
    assert "--output-root" in result.stdout
...
def test_recovery_report_file_action_local_requires_metadata() -> None:
    result = subprocess.run(
        [
            sys.executable,
            "-m",
...
def test_recovery_report_file_action_command_delegates_to_builder(
    capsys,
    monkeypatch,
) -> None:
    received: dict[str, object] = {}
    action_module = "async_scholar.session_window_recovery_report_file_action"
    fake_action_module = types.ModuleType(action_module)
    ...
    def fake_build(
        ...
        return {
            ...
            "relative_path": "stored-session-window-recovery-report.md",
            "preview_action": "none",
            "preview_reason": "report_already_current",
            "outcome": "no_action",
            "bytes_written": 0,
        }

    fake_action_module.__dict__[
        "build_stored_session_window_recovery_report_file_action"
    ] = fake_build
    monkeypatch.setitem(sys.modules, action_module, fake_action_module)
...
def test_session_window_recovery_report_file_action_sanitizes_build_failure(
    capsys,
    monkeypatch,
) -> None:
    action_module = "async_scholar.session_window_recovery_report_file_action"
...
def test_session_window_report_file_action_misordered_uses_action_error() -> None:
    result = subprocess.run(
        [
            sys.executable,
            "-m",
...
def test_session_window_recovery_report_file_action_handler_stays_thin() -> None:
    source = inspect.getsource(
        cli._run_session_window_recovery_report_file_action_local_command
    )
...
def test_session_window_recovery_review_batch_handler_stays_thin() -> None:
    source = inspect.getsource(
        cli._run_session_window_recovery_review_batch_local_command
    )
...
def _assert_session_window_stop_execute_output_is_safe(
    stdout: str,
    stderr: str,
    runtime_path: Path | None = None,
) -> None:
...
def _assert_session_window_stop_execution_preflight_output_is_safe(
    stdout: str,
    stderr: str,
) -> None:
    combined_output = f"{stdout}\n{stderr}".lower()
...
def _assert_gate_d_signal_quality_evidence_output_is_safe(
    stdout: str,
    stderr: str,
) -> None:
    payload = json.loads(stdout)
    ...
    assert payload["autonomous_participation_performed"] is False
    assert payload["academic_answer_behavior_performed"] is False
    assert set(payload) == set(EXPECTED_GATE_D_SIGNAL_QUALITY_EVIDENCE)
...
def _assert_gate_d_product_judgment_evidence_output_is_safe(
    stdout: str,
    stderr: str,
) -> None:
    payload = json.loads(stdout)
    ...
    assert payload["autonomous_participation_performed"] is False
    assert payload["academic_answer_behavior_performed"] is False
    assert set(payload) == set(EXPECTED_GATE_D_PRODUCT_JUDGMENT_EVIDENCE)
...
def _assert_gate_d_product_judgment_packet_output_is_safe(
    stdout: str,
    stderr: str,
) -> None:
    payload = json.loads(stdout)
...
def _assert_gate_d_security_review_evidence_output_is_safe(
    stdout: str,
    stderr: str,
) -> None:
    payload = json.loads(stdout)
    ...
    assert payload["autonomous_participation_performed"] is False
    assert payload["academic_answer_behavior_performed"] is False
    assert set(payload) == set(EXPECTED_GATE_D_SECURITY_REVIEW_EVIDENCE)
...
def _assert_gate_d_scheduler_lifecycle_evidence_output_is_safe(
    stdout: str,
    stderr: str,
) -> None:
    payload = json.loads(stdout)
    ...
    assert payload["autonomous_participation_performed"] is False
    assert payload["academic_answer_behavior_performed"] is False
    assert set(payload) == set(EXPECTED_GATE_D_SCHEDULER_LIFECYCLE_EVIDENCE)
...
def _assert_gate_d_local_evidence_bundle_output_is_safe(
    stdout: str,
    stderr: str,
) -> None:
    payload = json.loads(stdout)
    ...
    assert payload["gate_d_pass_claimed"] is False
    assert payload["product_promise_alpha_pass_claimed"] is False
    assert set(payload) == set(EXPECTED_GATE_D_LOCAL_EVIDENCE_BUNDLE)
